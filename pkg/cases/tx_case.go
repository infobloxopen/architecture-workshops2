...
func (tc *TxCase) Handle(w http.ResponseWriter, r *http.Request) {
	...
	// LAB: STEP2 FIXED - Move the slow network call OUTSIDE the transaction
	// to avoid holding DB connections and row locks during I/O.
	_, depErr := depclient.Call(r.Context(), tc.DepClient, "2s", "0.0")
	if depErr != nil {
		log.Printf("tx: dep call error: %v", depErr)
	}

	// Begin transaction - now only for the actual DB operations
	tx, err := tc.DB.Begin()
	if err != nil {
	...
	// LAB: STEP2 FIXED - Lock row, update, and commit quickly
	var balance int
	...
	// Update the row - transaction is held only for this brief moment
